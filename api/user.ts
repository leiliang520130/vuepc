...
// 绑定邮箱
export function userBindEmail(params: any, headers?: any) {
    return $request.post(
        { url: '/user/bindEmail', params, headers },
        { withToken: !headers?.token }
    )
}
...
export function shareInvite(params: Record<string, any>) {
    return $request.post({ url: '/share/bind', params })
}
...
export function updateUser(params?: Record<string, any>) {
    return $request.post({ url: '/login/updateUser', params })
}
